...
function TAFaqManage() {
  ...
  const fetchFaqs = async () => {
    try {
      ...
      const response = await axios.get('http://localhost:8000/faqs', config);
  ...
  const handleOpenCreate = () => {
    setIsEditMode(false);
    setFormData({ question: "", answer_html: "" });
    setFile(null);
    setShowModal(true);
  };
  ...
  const handleOpenEdit = (item) => {
    setIsEditMode(true);
    setTargetId(item.id);
    setFormData({ question: item.question, answer_html: item.answer_html });
    setFile(null);
    setShowModal(true);
  };

  const handleSave = async () => {
    if (!formData.question || !formData.answer_html) {
    ...
    try {
      ...
      if (isEditMode) {
        await axios.put(`http://localhost:8000/faqs/${targetId}`, sendData, config);
        alert("수정되었습니다.");
      } else {
        await axios.post(`http://localhost:8000/faqs`, sendData, config);
        alert("등록되었습니다.");
      }
      setShowModal(false);
      fetchFaqs();
  ...
  const handleDelete = async (id) => {
    if (window.confirm("정말 삭제하시겠습니까?")) {
      const token = localStorage.getItem('token');
      try {
        await axios.delete(`http://localhost:8000/faqs/${id}`, {
  ...
  return (
    <div style={styles.pageContainer}>
      ...
      <div style={styles.header}>
        ...
        <h2 style={{margin: 0, fontSize: 'clamp(20px, 5vw, 24px)', color: 'white', fontWeight: '500'}}>FAQ 관리</h2>
                    ...
                    <div key={item.id} style={styles.card}>
                        <div style={styles.cardContent}>
                            <div style={styles.question}>
                                <span style={{color:'#003675', marginRight:'5px'}}>Q.</span> 
                                {item.question}
                                {item.original_filename && <span style={styles.fileIcon}> 📎</span>}
                            </div>
                            <div style={styles.answer}>
                                <span style={{color:'#666', marginRight:'5px', fontWeight:'bold'}}>A.</span>
                                {item.answer_html}
                            </div>
                        </div>
        ...
        <div style={modalStyles.overlay}>
          <div style={modalStyles.modal}>
            ...
            <div style={modalStyles.content}>
              ...
              <div style={modalStyles.inputGroup}>
                <label style={modalStyles.label}>첨부파일 (선택)</label>
                <input 
                  type="file" 
              ...
              <div style={modalStyles.inputGroup}>
                ...
                <textarea 
                  ...
                  value={formData.answer_html}
                  onChange={(e) => setFormData({...formData, answer_html: e.target.value})}
                />
              </div>
...
const modalStyles = {
  ...
  textarea: { 
    ...
    boxSizing: 'border-box', resize: 'none', fontSize: '15px',
    lineHeight: '1.5'
  },
  saveBtn: { 
