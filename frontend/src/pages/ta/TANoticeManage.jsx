...
function TANoticeManage() {
  ...
  const [filteredList, setFilteredList] = useState([]); // 필터링된 리스트 상태 분리
  ...
  const fetchNotices = async () => {
    try {
      ...
      setFilteredList(response.data); // 초기값 설정
    } catch (error) {
...
const styles = {
  ...
  card: { backgroundColor: 'white', padding: '20px', borderRadius: '16px', marginBottom: '12px', boxShadow: '0 2px 8px rgba(0,0,0,0.05)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '10px', border: '1px solid #e9ecef' },
...
const modalStyles = {
  ...
  modal: { width: '90%', maxWidth: '600px', maxHeight: '85%', backgroundColor: 'white', borderRadius: '16px', display: 'flex', flexDirection: 'column', overflow: 'hidden', boxShadow: '0 10px 40px rgba(0,0,0,0.2)' },
