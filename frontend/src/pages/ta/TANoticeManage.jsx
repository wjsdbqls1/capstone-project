...
function TANoticeManage() {
  ...
  const filteredList = notices.filter(item => 
    item.title.toLowerCase().includes(keyword.toLowerCase())
  );

  const getGradeText = (grade) => {
    if (grade === 0) return "전체 공지";
    return `${grade}학년`;
  };
  ...
  const getGradeBadgeStyle = (grade) => {
    switch (grade) {
        case 0: // 전체
            return { backgroundColor: '#37474f', color: 'white' }; 
        case 1: // 1학년 (초록)
  ...
  return (
    <div style={styles.pageContainer}>
      ...
      <div style={styles.header}>
        ...
        <h2 style={{margin: 0, fontSize: 'clamp(20px, 5vw, 24px)', color: 'white', fontWeight: '500'}}>공지사항 관리</h2>
...
const modalStyles = {
  ...
  select: { 
    width: '100%', padding: '12px', border: '1px solid #ccc', 
    borderRadius: '8px', fontSize: '15px', backgroundColor: 'white'
  },
  fileInput: { 
