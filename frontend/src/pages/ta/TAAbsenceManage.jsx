...
function TAAbsenceManage() {
  ...
  useEffect(() => {
    ...
    const result = requests.filter(req => {
      const reqDate = new Date(req.created_at); // 신청일 기준
      const reqDateStart = new Date(reqDate.getFullYear(), reqDate.getMonth(), reqDate.getDate());

      if (dateFilter === 'today') {
        return reqDateStart.getTime() === todayStart.getTime();
      }
      if (dateFilter === 'week') {
        ...
        {view === 'list' ? (
            <div style={styles.listArea}>
                {filteredRequests.length === 0 ? (
                  <div style={{textAlign:'center', marginTop:'50px', color:'#666', fontWeight:'500'}}>
                    {dateFilter !== 'all' ? "선택한 기간에 신청 내역이 없습니다." : "신청 내역이 없습니다."}
                  </div>
                ) : (
                  filteredRequests.map((req) => {
                    ...
                    return (
                        <div 
                          key={req.id} 
                          style={styles.card} 
                          onClick={() => { setSelectedReq(req); setView('detail'); setShowRejectInput(false); }}
                          onMouseOver={(e) => {
                        ...
                        >
                            <div style={{display:'flex', justifyContent:'space-between', marginBottom:'8px'}}>
                                <span style={{fontSize:'13px', color:'#888', fontWeight:'500'}}>{req.created_at.split('T')[0]}</span>
                            ...
                            <div style={{fontSize:'14px', color:'#555'}}>
                              <span style={{fontWeight:'600', color:'#003675'}}>{req.course_name}</span> | <span style={{color:'#c62828'}}>{req.absent_date}</span>
                            </div>
                        </div>
                    );
                })
            ...
            <div style={styles.detailContainer}>
                ...
                {selectedReq.status === 'SUBMITTED' ? (
                  ...
                  <div style={{
                      ...
                      border: selectedReq.status === 'APPROVED' ? '1px solid #c8e6c9' : '1px solid #ffcdd2'
                  }}>
...
const styles = {
  ...
  // 필터 바 스타일
  filterBar: { marginBottom:'15px', padding:'10px 15px', backgroundColor:'#f8f9fa', borderRadius:'12px', border:'1px solid #e9ecef', display:'flex', alignItems:'center' },
  select: { padding:'8px 12px', borderRadius:'8px', border:'1px solid #ced4da', backgroundColor:'white', fontSize:'14px', cursor:'pointer', outline:'none', minWidth:'100px' },
  ...
  // 섹션 박스
  sectionBox: { backgroundColor: 'white', padding: '25px', borderRadius: '16px', marginBottom: '20px', border: '1px solid #e0e0e0', boxShadow: '0 2px 8px rgba(0,0,0,0.03)' },
