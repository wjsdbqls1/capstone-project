...
function StudentHistory() {
  ...
  useEffect(() => {
    const fetchInquiries = async () => {
      ...
      try {
        const response = await axios.get('http://localhost:8000/inquiries/me', {
  ...
  const handleClickItem = async (item) => {
    ...
    try {
      const qRes = await axios.get(`http://localhost:8000/inquiries/${item.id}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      const rRes = await axios.get(`http://localhost:8000/inquiries/${item.id}/replies`, {
        headers: { Authorization: `Bearer ${token}` }
      });
                ...
                <>
                  ...
                  {detailData.attachment && (
                    <div style={modalStyles.section}>
                        ...
                        <a href={`http://localhost:8000${detailData.attachment}`} target="_blank" rel="noopener noreferrer" style={modalStyles.link}>
                            💾 다운로드 / 보기
                        </a>
                    </div>
                  )}
                        ...
                        <div key={reply.id} style={modalStyles.answerBox}>
                          ...
                          {reply.attachment && (
                            <div style={{marginTop:'10px', fontSize:'14px', borderTop:'1px dashed #a6cbf3', paddingTop:'5px'}}>
                                📎 <b>첨부파일:</b>
                                <a href={`http://localhost:8000${reply.attachment}`} target="_blank" rel="noopener noreferrer" style={{marginLeft:'5px', color:'#003675', fontWeight:'bold', textDecoration:'underline'}}>
                                    확인하기
                                </a>
                            </div>
                          )}
