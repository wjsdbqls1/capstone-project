...
class Notice(Base):
    ...
    target_grade = Column(Integer, nullable=False, default=0)  # 더 이상 앱에서 쓰지 않음(하위 호환용으로만 컬럼 유지)
    target_grades = Column(String(20), nullable=False, default="0", server_default="0")  # 콤마로 구분된 대상 학년(예: "0"=전체, "1,3")
